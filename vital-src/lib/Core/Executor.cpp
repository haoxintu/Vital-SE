...
// THX newly added
cl::opt<unsigned> PerSimulationInstructions(
    "per-simulation-instructions",
    cl::desc("Number of instructions to be executed per simulation (default=100)"),
    cl::init(100),
    cl::cat(TerminationCat));
...
// XXX hack
extern "C" unsigned dumpStates, dumpPTree, debug_thx;
unsigned dumpStates = 0, dumpPTree = 0;
// THX
unsigned debug_thx = 0;
extern std::set<std::string> globalRecordingSet;
extern std::set<llvm::Instruction*> globalUnsafeInstructionSet;
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    : Interpreter(opts), interpreterHandler(ih), searcher(0),
      ...
      atMemoryLimit(false), inhibitForking(false), haltExecution(false), //haltSimExecution(false), // THX added
...
void Executor::branch(ExecutionState &state,
  ...
  if (!branchingPermitted(state)) {
    ...
    for (unsigned i=1; i<N; ++i) {
      ...
      result.push_back(ns);
      if (debug_thx) llvm::errs() << "THX handling Switch: \n";
      if (ns->isSimulationState()){
        interpreterHandler->incNumSimulatedStates();
      }
     
      if (!is_simulation_mode){ // THX added for not adding node to the tree in simuation mode
        ;//es->ptreeNode->isInMCTSTree = 1;
      }
...
Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
  ...
  if (res==Solver::True) {
    ...
    if (current.isNormalState()){
        ++stats::forks;
    }
    if (current.isSimulationState() && is_simulation_mode) {
        llvm::errs() << "fork here?" << " is_simulation_mode = " << is_simulation_mode << "\n";
        exit(1);
        ++stats::forksSimulation;
    }

    falseState = trueState->branch();
    addedStates.push_back(falseState);

    // THX add statictic for simulated states
    if (trueState->isSimulationState())
        interpreterHandler->incNumSimulatedStates();
    ...
    if (debug_thx) llvm::errs() << "current.ptreeNode in fork function = " << current.ptreeNode << "\n";

    // THX set the flags of fork, so that the state can be switched now
    if (!is_simulation_mode)
        current.ptreeNode->isForked = 1;

    if (debug_thx){
        llvm::errs() << "State ID in fork : " << current.id << " fork at node : " << current.ptreeNode << "\n";
    }
...
long long int inst_count_per_simulation = 0;
void Executor::stepInstruction(ExecutionState &state) {
  if (state.isNormalState()){
    printDebugInstructions(state);
    if (statsTracker) {
      //if (!is_simulation_mode) // THX added
    ...
    ++stats::instructionsSimulation;
    //llvm::errs() << "instructionsSimulation : " << stats::instructionsSimulation << "\n";
    ++inst_count_per_simulation;
  }
  ...
  if (stats::instructions == MaxInstructions)
    haltExecution = true;
      ...
      //llvm::errs() << "@@@@ Simulated instructions per simulation = " << inst_count_per_simulation << "\n";
      /*
      if (inst_count_per_simulation == PerSimulationInstructions){ // set maxsium simulation instrucitons
          //haltSimExecution = true;
        if (debug_thx) llvm::errs() << "~~~~~~~ The simutation run should be stopped here: reach the maximum simulated instructions\n";
        //is_simulation_mode = 0;
        //state.getDependentState()->ptreeNode->reward = getSimulationReward(memoryOperationInstructionSet, &state); // too late to update the reward
        //double reward = getSimulationReward(memoryOperationInstructionSet, &state);
        //double reward = getSimulationReward(memoryOperationInstructionSet, state.getDependentState());
        if (debug_thx) llvm::errs() << "////////////// (doSimulation Run End) for " << state.getDependentState()->ptreeNode << "...\n";
        //llvm::errs() << "////////////// (doSimulation Run End) for ...\n";
        if (debug_thx) {
            state.getDependentState()->pc->inst->dump();
            llvm::errs() << "~~~ Size of addedStates = " << addedStates.size() << "\n";
            llvm::errs() << "~~~ Size of removedStates = " << removedStates.size() << "\n";
            llvm::errs() << "~~~ Size of states = " << states.size() << "\n";
            llvm::errs() << "~~~ Size of suspendStates = " << suspendedStates.size() << "\n";
            for(auto es : suspendedStates){
                llvm::errs() << "   es->pc->inst : " << es->pc->inst << "\n";
                llvm::errs() << "   es->ptreeNode : " << es->ptreeNode << "\n";
                llvm::errs() << "   es->ptreeNode->left : " << es->ptreeNode->left.getPointer() << "\n";
                llvm::errs() << "   es->ptreeNode->right : " << es->ptreeNode->right.getPointer() << "\n";
            }
            llvm::errs() << "~~~ Size of resumedStates = " << resumedStates.size() << "\n";
        }
        //onSimulationStateExit(state);
        //doBackpropagation(reward, state.getDependentState()->ptreeNode);
        //doBackpropagation(reward, state.ptreeNode);
        //onSimulationStateExit(state);
        //greward = 0;
        terminateStateEarly(state, "mcts-limit exceeded.", StateTerminationType::Interrupted);
        memoryOperationInstructionSet.clear(); // renew the set for every simulation run
        inst_count_per_simulation = 0;
        //haltExecution = true;
      }
      */
  }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      // THX
      //state.ptreeNode->isForked = 1;
      // THX: if the fork is executed, the current PTreeNode should not be terminal, so set the flag here
      //state.ptreeNode->isTerminal = 0;
      //branches.first->ptreeNode->isInMCTSTree = 0;
      //branches.second->ptreeNode->isInMCTSTree = 0;
  ...
  case Instruction::Load: {
    ...
    if (location.get()) {
        std::string record = "LOAD-" + i->getFunction()->getName().str() + "_nesCheck"
            + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location.getCol());
        if (globalRecordingSet.count(record)) { // the element is in the set, count is 1
            if(unsafeMemoryOperationRecords.find(record) == unsafeMemoryOperationRecords.end()){
                unsafeMemoryOperationRecords[record] = 1;
            }else{
                unsafeMemoryOperationRecords[record] += 1;
            }
        }
  ...
  case Instruction::Store: {
    ...
    const llvm::DebugLoc &location = i->getDebugLoc();
    if (location.get()) {
        std::string record = "STORE-" + i->getFunction()->getName().str() + "_nesCheck"
        + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location.getCol());
        if (globalRecordingSet.count(record)) { // the element is in the set, count is 1
            if(unsafeMemoryOperationRecords.find(record) == unsafeMemoryOperationRecords.end()){
                unsafeMemoryOperationRecords[record] = 1;
  ...
  case Instruction::GetElementPtr: {
    ...
    const llvm::DebugLoc &location = i->getDebugLoc();
      if (location.get()) {
        std::string record = "GETE-" + i->getFunction()->getName().str() + "_nesCheck"
                + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location.getCol());
        if (globalRecordingSet.count(record)) { // the element is in the set, count is 1
            if(unsafeMemoryOperationRecords.find(record) == unsafeMemoryOperationRecords.end()){
                unsafeMemoryOperationRecords[record] = 1;
  ...
  case Instruction::BitCast: {
    ...
    const llvm::DebugLoc &location = i->getDebugLoc();
      if (location.get()) {
        std::string record = "CAST-" + i->getFunction()->getName().str() + "_nesCheck"
            + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location.getCol());
        if (globalRecordingSet.count(record)) { // the element is in the set, count is 1
            if(unsafeMemoryOperationRecords.find(record) == unsafeMemoryOperationRecords.end()){
                unsafeMemoryOperationRecords[record] = 1;
...
void Executor::updateStates(ExecutionState *current) {
  ...
  for (std::vector<ExecutionState *>::iterator it = removedStates.begin(),
    ...
    std::map<ExecutionState*, std::vector<SeedInfo> >::iterator it3 =
      seedMap.find(es);
    if (it3 != seedMap.end()){
        //llvm::errs() << "erase here? "<< "\n";
      seedMap.erase(it3);
    }
    if (haltExecution == true){
        if (es->ptreeNode->left.getPointer() == 0 && es->ptreeNode->right.getPointer() == 0){
            processTree->remove(es->ptreeNode);
            delete es;
        } else {
            ;// TODO what are the consequence?
        }
    }else{
    processTree->remove(es->ptreeNode);
    delete es;
...
void Executor::run(ExecutionState &initialState) {
  if (is_simulation_mode)
      llvm::errs() << "!!! In Simuation mode run\n";
  ...
  //} // end checking for is_simulation_mode
  ...
  while (!states.empty() && !haltExecution) {
    ...
    if (is_simulation_mode){
        //state.pc->inst->dump();
        collectInstructionsInSimulation(state.pc->inst);
    }
    ...
    if (!is_simulation_mode){
        state.ptreeNode->isInMCTSTree = 1; // must set before update, as the value is updated in update function
    }

    updateStates(&state);


    if (!checkMemoryUsage() && !is_simulation_mode) {
  ...
  searcher = nullptr;
    ...
    std::string file_name = interpreterHandler->getOutputFilename("unsafe-pt.txt");
    if (file_name.size() == 0){
        klee_warning("open file failed of malloc functions");
        exit(1);
    }
    std::ofstream f(file_name);
    std::map<std::string, unsigned long long> result_records = getUnsafeMemoryOperationRecords();
    for (auto record : result_records){
        f << record.first << "\t" << record.second << "\n";
    }
    f.close();
    // End for the adding
  doDumpStates();
...
void Executor::doBackpropagation(double reward, PTreeNode* node){
    if(debug_thx) llvm::errs() << "@@@@@@@@@@@@@@ (doBackpropagation Start) ... reward of current node = " << node->reward << "\n";
    while (node){
       node->visit += 1;
       node->reward += reward;
       node = node->parent;
    }
    if(debug_thx) llvm::errs() << "@@@@@@@@@@@@@@ (doBackpropagation End) ...\n";
    return;
}
void Executor::terminateState(ExecutionState &state) {
  ...
  if (state.isNormalState()) {
    ...
    interpreterHandler->incPathsExploredSimulation();
  }
  ...
  if (!is_simulation_mode && state.isNormalState()){
    state.ptreeNode->isTerminal = 1;
  }
  if (debug_thx){
    if (!is_simulation_mode && state.isNormalState()){
      ;;//dumpPTree();
    ...
    if (! is_simulation_mode && state.isNormalState()){
        llvm::errs() << "Sleep .... at node : " << state.ptreeNode << "\n";
        sleep(2);
    }
  }
  // THX just return? no need to produce test cases for simulation mode
  // This should make sense as the normal random path exploration can hardly detect bug.
  // THX change the flag here
  if (is_simulation_mode){
    if (debug_thx) llvm::errs() << "~~~~~~~ The simutation run should be stopped here: terminateState\n";
    is_simulation_mode = 0;
    double reward = getSimulationReward(memoryOperationInstructionSet, &state);
    ...
    return; // TODO any side effects if I just return?
  }
}
...
void Executor::terminateStateOnExit(ExecutionState &state) {
  ...
  if (state.isNormalState() && !is_simulation_mode){
    interpreterHandler->incPathsCompleted();
  }
  else {
    interpreterHandler->incPathsCompletedSimulation();
  }
...
void Executor::terminateStateOnError(ExecutionState &state,
                                     ...
                                     const char *suffix) {// THX just return? no need to produce test cases for simulation mode
  
  // count the error for the reward
  if ((state.isSimulationState()) && (! state.isNormalState()) && (terminationType == StateTerminationType::Abort ||
          terminationType == StateTerminationType::Ptr)){
         // terminationType == StateTerminationType::Overflow || terminationType == StateTerminationType::InvalidLoad)){
    state.errorCount += 1;
  }
  ...
  if (EmitAllErrors ||
    ...
    // THX remove the checking to save more errors
    if (!is_simulation_mode) {
...
void Executor::runFunctionAsMain(Function *f,
  ...
  processTree = new PTree(state); // THX
  run(*state);
  processTree = nullptr;
...
void Executor::startSimulationState(ExecutionState &state, SimulationInfo* simulationInfo) {

  ExecutionState *snapshotState = simulationInfo->snapshot->state;

  /* TODO: non-first snapshots hold normal state properties! */

  /* initialize recovery state */
  ExecutionState *simulationState = new ExecutionState(*snapshotState);
  //llvm::errs() << "state was set = " << simulationState << "\n";
  simulationState->setType(SIMULATION_STATE);

  /* TODO set mcts instruction seems no need? */
  ...
  /* link the current state to it's simulation state */
  state.setSimulationState(simulationState);

  /* update process tree */
  simulationState->ptreeNode = state.ptreeNode;

  /* add the recovery state to the searcher */
  addedStates.push_back(simulationState);

  /* update statistics */
  interpreterHandler->incNumSimulatedStates();
  interpreterHandler->incNumInitialSimulatedStates();

  /* option: set the searcher do not fork */
  inhibitForking = 1;
}
void Executor::resumeState(ExecutionState &state, bool implicitlyCreated) {
  state.setResumed();
  state.setSimulationState(0);
  addedStates.push_back(&state);
}
void Executor::onSimulationStateExit(ExecutionState &state) {

  //ExecutionState *dependentState = state.getDependentState();
  //dumpConstrains(*dependentState);
  notifyDependentState(state);
  //terminateState(state);
  terminateStateEarly(state, "max-depth exceeded.", StateTerminationType::Interrupted);
  // terminates other simulated alive states
  for (auto es : states) {
    if(debug_thx) llvm::errs() << " es : " << es << "\n";
    if(debug_thx) llvm::errs() << " es->ptreeNode : " << es->ptreeNode << "\n";
    if(debug_thx) llvm::errs() << " es->isSimulationState : " << es->isSimulationState() << "\n";
    if(debug_thx) llvm::errs() << " es->isNormalState() : " << es->isNormalState() << "\n";
    if(debug_thx) llvm::errs() << " +++++++++++++++++ " << "\n";
    if (es->isSimulationState() || ! es->isNormalState()){
        if(debug_thx) {
            llvm::errs() << "before terminated es->ptreeNode : " << es->ptreeNode << " ; es : " << es << "\n";
            llvm::errs() << "       left = " << es->ptreeNode->left.getPointer() <<
                " right = " << es->ptreeNode->right.getPointer() << " isNormalState() = " << es->ptreeNode->state->isNormalState()
                << " isSimulationState() = " << es->ptreeNode->state->isSimulationState() << "\n";
        }
        
        terminateStateEarly(*es, "max-depth exceeded.", StateTerminationType::Interrupted);
        
        if(debug_thx) {
            llvm::errs() << "after terminated es->ptreeNode : " << es->ptreeNode << " ; es : " << es << "\n";
            llvm::errs() << "       left = " << es->ptreeNode->left.getPointer() <<
                " right = " << es->ptreeNode->right.getPointer() << " isNormalState() = " << es->ptreeNode->state->isNormalState()
                << " isSimulationState() = " << es->ptreeNode->state->isSimulationState() << "\n";
        }
  ...
  /* option: set the searcher do fork again */
  inhibitForking = 0;
}

void Executor::notifyDependentState(ExecutionState &simulationState) {
  ExecutionState *dependentState = simulationState.getDependentState();
  simulationState.ptreeNode->state = dependentState;
  if (0) {
    llvm::errs() << "current dependentState pc->inst : ";
    dependentState->pc->inst->dump();
    llvm::errs() << "mctsInst in dependentState : ";
  
    llvm::errs() << "dependentState = " << dependentState << "\n";
    llvm::errs() << "dependentState->ptreeNode = " << dependentState->ptreeNode << "\n";
  }
...
double Executor::getSimulationReward(std::set<llvm::Instruction*> current_records, ExecutionState* state){
  double ret = 0;
  unsigned int count_unsafe = 0;
  unsigned int count_error = 0;
  if (debug_thx) llvm::errs() << "++++++++++++++++ Now caculate the reward!!! current_records size = " << current_records.size() << "\n";
  if (debug_thx) llvm::errs() << "globalRecordingSet size in getSimulationReward() = " << globalRecordingSet.size() << "\n";
  for (auto re : globalRecordingSet){
    if (debug_thx) llvm::errs() << re << "\n";
  }
  for (auto ii : current_records){
    if (globalUnsafeInstructionSet.count(ii)){ // if the instruction is in the set, the .count returns 1
      count_unsafe += 1;
      continue;
    }
    // not in the set, so keep looking
    if (isa<StoreInst>(ii)) {
      const llvm::DebugLoc &location = ii->getDebugLoc();
      if (location.get()) {
        ...
        if (globalRecordingSet.count(record)) { // the element is in the set, count is 1
            count_unsafe += 1;
            globalUnsafeInstructionSet.insert(ii);
        }
      }
    }
    if (isa<LoadInst>(ii)){
  ...
  count_error = state->errorCount;
  ret = 1 * count_unsafe + 1 * count_error;
  simulationReward = ret;

  if (debug_thx) llvm::errs() << "reward in getSimulationReward before return = " << ret << "\n";
  return ret;
}


void Executor::collectInstructionsInSimulation(llvm::Instruction* inst) {
  if (isa<StoreInst>(inst) || isa<LoadInst>(inst) || isa<GetElementPtrInst>(inst) || isa<CastInst>(inst))
    memoryOperationInstructionSet.insert(inst);
}


std::map<std::string, unsigned long long> Executor::getUnsafeMemoryOperationRecords(){
   return unsafeMemoryOperationRecords;
}
