...
public:
  ...
  bool suspendStatus;
  /* a normal state has a simulation recovery state */
  ExecutionState *simulationState;
  int errorCount;


  // a simulation state properties
  /* a simulation state must reset this pc when the simulation run is done */
  llvm::Instruction *mctsInst;
  ...
  // THX added for specify simulation run or normal run
...
public:
  ...
  ExecutionState(const ExecutionState &state); // THX moved here

  ExecutionState *branch();

  //std::set<std::string> unsafeMemoryOperationSetPerState; // THX
  ...
  bool isSimulationState() {
    return (type & SIMULATION_STATE) != 0;
  }
  ...
  ExecutionState *getSimulationState() {
    assert(isNormalState());
    return simulationState;
  }

  void setSimulationState(ExecutionState *state) {
    assert(isNormalState());
    if (state) {
      assert(state->isSimulationState());
    }
    simulationState = state;
  }

  llvm::Instruction *getMctsInst() {
    assert(isNormalState());
    return mctsInst;
  }

  void setMctsInst(llvm::Instruction *mctsInst) {
    assert(isNormalState());
    this->mctsInst = mctsInst;
  }

  unsigned int getLevel() {
    assert(isSimulationState());
    return level;
  }
