...
public:
  ...
  RNG theRNG;
  //void run(ExecutionState &initialState); // THX moved here
  //std::vector<ExecutionState*> mcts_children; // THX
  std::set<ExecutionState*, ExecutionStateIDCompare> states; // THX
  //std::unique<PTree> processTree;
  PTree* processTree; // THX
  //std::unique_ptr<PTree> processTree_sim; //THX
  //PTree* processTree; // THX
  //PTree* processTree_sim; //THX
  //PTree* processTree_mcts; //THX
  //PTree* processTree_sim; //THX
  unsigned int UnsafeOperationCount = 0;
  std::set<llvm::Instruction*> memoryOperationInstructionSet;
  void terminateState(ExecutionState &state); //                  TODO thx
  //std::set<std::string> unsafeMemoryOperationSetPerState;
private:
  std::unique_ptr<KModule> kmodule;
  InterpreterHandler *interpreterHandler;
  Searcher *searcher;
  //Searcher *searcher_mcts;
  ...
  //std::unique_ptr<MemoryManager> memory_mcts;
  //std::set<ExecutionState*, ExecutionStateIDCompare> states_backup_mcts; // THX
  //std::map<unsigned int, KInstIterator> map_mcts_pc;
  //std::map<unsigned int, KInstIterator> map_mcts_prevPC;
  //std::map<unsigned int, std::vector<StackFrame>> map_mcts_stack;
  //typedef ImmutableMap<const MemoryObject*, ObjectHolder, MemoryObjectLT> MemoryMap;
  //std::map<unsigned int, MemoryMap> map_mcts_addrspace_objects;
  //std::map<unsigned int, unsigned> map_mcts_addrspace_cowkey;
  //std::map<unsigned int, std::vector<ref<Expr>>> map_mcts_constraints;
  //std::map<unsigned int, SolverQueryMetaData> map_mcts_queryMetaData;
  //std::map<unsigned int, PTreeNode*> map_mcts_ptreeNode;
  //std::map<unsigned int, Searcher*> map_mcts_searcher;
  StatsTracker *statsTracker;
  TreeStreamWriter *pathWriter, *symPathWriter;
  SpecialFunctionHandler *specialFunctionHandler;
  TimerGroup timers;
  //std::unique_ptr<PTree> processTree;
  //std::unique_ptr<PTree> processTree_sim;
  ...
  std::vector<ExecutionState *> addedStates_backup_mcts; // THX
  /// Used to track states that have been removed during the current
  /// instructions step.
  /// \invariant \ref removedStates is a subset of \ref states.
  /// \invariant \ref addedStates and \ref removedStates are disjoint.
  std::vector<ExecutionState *> removedStates;
  std::vector<ExecutionState *> removedStates_backup_mcts;
  std::map<std::string, unsigned long long> unsafeMemoryOperationRecords; // THX
...
public:
  ...
  void startSimulationState(ExecutionState &state, SimulationInfo* simulationInfo);
  void resumeState(ExecutionState &state, bool implicitlyCreated);
  void notifyDependentState(ExecutionState &simulationState);
  void onSimulationStateExit(ExecutionState &state);
