...
extern bool enable_simulation; //
extern unsigned int simulation_limit; //
extern bool simulation_opt;
...
namespace klee {
  ...
  // THX added for MCTS algorithm
  class MCTSSearcher final : public Searcher {
    PTree &processTree;
    Executor &executor;
    std::vector<ExecutionState*> children;
  ...
  public:
    /// \param processTree The process tree.
    /// \param RNG A random number generator.
    MCTSSearcher(PTree &processTree, Executor &_executor, RNG &rng);
    ~MCTSSearcher() override = default;
    ...
    // THX MCTS functions
    bool isTerminalNode(PTreeNode *);
    bool hasEligiableChildren(PTreeNode* node);
    PTreeNode* doSelection(PTreeNode*);
    PTreeNode* doExpansion(PTreeNode*, std::vector<ExecutionState*>);
    std::vector<std::vector<llvm::BasicBlock*>> getBranchBlocks(ExecutionState* left, ExecutionState* right);
    std::vector<double> getExpansionScore(std::vector<std::vector<llvm::BasicBlock*>>, ExecutionState*, ExecutionState*, std::set<std::string>);
    std::vector<PTreeNode*> getChildren(PTreeNode* node);
    double computeUCT(PTreeNode* parent, PTreeNode* child);
    PTreeNode* selectBestChildren(PTreeNode* current_node, PTreeNode* left, PTreeNode* right);
    PTreeNode* expandBestChildren(PTreeNode* left, PTreeNode* right);
    double doSimulation(PTreeNode* node);
    void doBackpropagation(double reward, PTreeNode* node);
    bool shouldDoSimulation(PTreeNode* node);
    int getUnsafeCountPerBB(llvm::BasicBlock*);
  };

  class SplittedSearcher : public Searcher {
    Searcher *mctsSearcher;
    Searcher *simulationSearcher;
    unsigned int sim_mode;

  public:
    SplittedSearcher(Searcher *baseSearcher, Searcher *recoverySearcher, unsigned int sim_mode);
    ~SplittedSearcher();
    ...
    void printName(llvm::raw_ostream &os) {
      os << "- Meta Splitted Searcher (used to switch the following base and simulation searcher)\n";
      os << "- Base Searcher: "; mctsSearcher->printName(os);
      os << "- Simulation Searcher searcher: "; simulationSearcher->printName(os);
    }
  };

  class RandomSimulationPath : public Searcher {
    Executor &executor;
  ...
  public:
    RandomSimulationPath(Executor &executor);

    ~RandomSimulationPath();
