...
private:
  ...
  unsigned m_pathsCompletedSimulation; // number of completed paths for simulation THX
  unsigned m_pathsExploredSimulation; // number of partially explored and completed paths for simulation THX
  unsigned m_numSimulatedStates; // number of simulated executed states THX
  unsigned m_numInitialSimulatedStates; // number of simulated executed states THX
...
public:
  ...
  // THX start
  unsigned getNumPathsCompletedSimulation() { return m_pathsCompletedSimulation; }
  unsigned getNumPathsExploredSimulation() { return m_pathsExploredSimulation; }
  void incPathsCompletedSimulation() { ++m_pathsCompletedSimulation; }
  void incPathsExploredSimulation(std::uint32_t num = 1) {
    m_pathsExploredSimulation += num; }

  unsigned getNumStatesSimulated() {
    return m_numSimulatedStates;
  }
  void incNumSimulatedStates() {
    m_numSimulatedStates++;
  }
  unsigned getNumInitialStatesSimulated() {
    return m_numInitialSimulatedStates;
  }
  void incNumInitialSimulatedStates() {
    m_numInitialSimulatedStates++;
  }
  // THX end
...
extern "C" unsigned debug_thx;
/* Outputs all files (.ktest, .kquery, .cov etc.) describing a test case */
void KleeHandler::processTestCase(const ExecutionState &state,
                                  const char *errorMessage,
                                  const char *errorSuffix) {
  if (debug_thx){
    llvm::errs() << "THX testing in processTestCase\n";
  }
  if (!WriteNone) {
    ...
    std::set<std::string> result_set = m_interpreter->getUnsafeMemoryOperationSet(state);
    ///llvm::errs() << "size of result_set = " << result_set.size() << "\n";
...
int main(int argc, char **argv, char **envp) {
  ...
  llvm::errs() << "Debug: KLEE-MCTS Version\n\n";
  if (Watchdog) {
    if (MaxTime.empty()) {
  ...
  std::map<std::string, std::vector<unsigned> > errorLocationOptions; // THX added from chopper
  parseErrorLocationParameter(ErrorLocation, errorLocationOptions); // THX added from chopper
  ...
  IOpts.errorLocations = errorLocationOptions; // THX added from chopper
  ...
  // THX added for MCTS
  /*
    std::string file_name = handler->getOutputFilename("unsafe-pt.txt");
    if (file_name.size() == 0){
        klee_warning("open file failed of malloc functions");
        exit(1);
    }
    std::ofstream f(file_name);
    //f << "record \t \t \t frequence \n";
    std::map<std::string, unsigned long long> result_records = interpreter->getUnsafeMemoryOperationRecords();
    ///llvm::errs() << "size of result_set = " << result_set.size() << "\n";
  ...
  // THX
  uint64_t instructionsSimulation =
    *theStatisticManager->getStatisticByName("InstructionsSimulation");
  uint64_t forksSimulation =
    *theStatisticManager->getStatisticByName("ForksSimulation");
  ...
  stats << '\n'
        << "Statistics for simulation execution: \n"
        << "KLEE: simulation done: initial states = " << handler->getNumInitialStatesSimulated() << '\n'
        << "KLEE: simulation done: total states = " << handler->getNumStatesSimulated() << '\n'
        << "KLEE: simulation done: total instructions = " << instructionsSimulation << '\n'
        << "KLEE: simulation done: total forks = " << forksSimulation << '\n'
        << "KLEE: simulation done: completed paths = " << handler->getNumPathsCompletedSimulation()
        << '\n'
        << "KLEE: simulation done: partially completed paths = "
        << handler->getNumPathsExploredSimulation() - handler->getNumPathsCompletedSimulation()
        << '\n';
        // TODO what else do we want to know?
