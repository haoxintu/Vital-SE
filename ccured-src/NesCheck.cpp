				...
				if (valoperand->getType()->isPointerTy())
				{
					...
					if (!differentBasicBlock)
					{
						if (varinfo != NULL)
						{
                            ...
                            std::string func_name = II->getFunction()->getName().str();
                            const DebugLoc &location = II->getDebugLoc();
                            //errs() << "location: " << location << "\n";
                            if (location.get() != NULL){ // TODO why the location is NULL with -O0?
                                ...
                                //std::string record = location->getFilename().str() + "-STORE-" + II->getFunction()->getName().str()
                                ...
                                if (type == "SEQ"){
                                    errs() << "marking" << "\n";
                                    //std::string record = location->getFilename().str() + "-STORE-SEQ-" + II->getFunction()->getName().str()
                                    //std::string record = location->getFilename().str() + "-STORE-" + II->getFunction()->getName().str()
                                    std::string record = "STORE-" + func_name
                                        + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location->getColumn());
                                    errs() << " **** record : " << record << "\n";
                                ...
                                if (type == "DYN"){
                                    errs() << "marking" << "\n";
                                    //std::string record = location->getFilename().str() + "-STORE-DYN-" + II->getFunction()->getName().str()
                                    //std::string record = location->getFilename().str() + "-STORE-" + II->getFunction()->getName().str()
                                    std::string record = "STORE-" + func_name
                                        + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location->getColumn());
                                    errs() << " **** record : " << record << "\n";
                                    errs() << " Function name : " << II->getFunction()->getName().str() << "\n";
                                    records.insert(record);
                                }
                            }
                            // THX ends
				...
				if (II->getType()->isPointerTy())
				{
					...
					if (varinfo != NULL)
					{
                        ...
                        if (location.get() != NULL){ // TODO why the location is NULL with -O0?
                            ...
                            errs() << " ### source code line : " << location.getLine() << "\n";
                            errs() << " ### source code column : " << location->getColumn() << "\n";
                            //std::string record = location->getFilename().str() + "-LOAD-" + II->getFunction()->getName().str()
                            //       + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location->getColumn());
                            //errs() << " **** record : " << record << "\n";
                            std::string type = PtrTypeToString(varinfo->classification);
                            if (type == "SEQ"){
                                errs() << "marking" << "\n";
                                //std::string record = location->getFilename().str() + "-LOAD-SEQ-" + II->getFunction()->getName().str()
                                //std::string record = location->getFilename().str() + "-LOAD-" + II->getFunction()->getName().str()
                                std::string record = "LOAD-" + func_name
                                    + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location->getColumn());
                                errs() << " **** record : " << record << "\n";
                                records.insert(record);
                            }
                            if (type == "DYN"){
                                errs() << "marking" << "\n";
                                //std::string record = location->getFilename().str() + "-LOAD-DYN-" + II->getFunction()->getName().str()
                                //std::string record = location->getFilename().str() + "-LOAD-" + II->getFunction()->getName().str()
                                std::string record = "LOAD-" + func_name
                                    + "-" + std::to_string(location.getLine()) + "-" + std::to_string(location->getColumn());
                                errs() << " **** record : " << record << "\n";
			...
			else if (GetElementPtrInst *II = dyn_cast_or_null<GetElementPtrInst>(I))
			{
				...
				if (!(II->hasAllZeroIndices())){
                    ...
                    if (location.get() != NULL){ // TODO why the location is NULL with -O0?
                        ...
                        errs() << " ### source code line : " << location.getLine() << "\n";
                        errs() << " ### source code column : " << location->getColumn() << "\n";
                        //std::string record = location->getFilename().str() + "-GETE-SEQ-" + II->getFunction()->getName().str()
                        //std::string func_name = II->getFunction()->getName().str();
                        //int start_position_to_erase = func_name.find("_nesCheck");
                        //func_name.erase(start_position_to_erase, 7);
                        //std::string record = location->getFilename().str() + "-GETE-" + II->getFunction()->getName().str()
                        std::string record = "GETE-" + func_name
                            + "-" + std::to_string(location.getLine()) + "-" +  std::to_string(location->getColumn());
                        errs() << " **** record : " << record << "\n";
                        //errs() << " Function name : " << func_name << "\n";
                        records.insert(record);
                        }
                    // THX ends
                }
					...
					if (countIndirections(srcT) != countIndirections(dstT) || innerSrcT->isIntegerTy() != innerDstT->isIntegerTy())
					{
						if (LoadInst *III = dyn_cast_or_null<LoadInst>(II->getOperand(0))) {
                            ...
                            if (location.get() != NULL){ // TODO why the location is NULL with -O0?
                                ...
                                errs() << " ### source code line : " << location.getLine() << "\n";
                                errs() << " ### source code column : " << location->getColumn() << "\n";
                                //std::string record = location->getFilename().str() + "-CAST-DYN-" + II->getFunction()->getName().str()
                                //std::string record = location->getFilename().str() + "-CAST-" + II->getFunction()->getName().str()
                                std::string record = "CAST-" + func_name
                                    + "-" + std::to_string(location.getLine()) + "-" +  std::to_string(location->getColumn());
                                errs() << " **** record : " << record << "\n";
                                records.insert(record);
                            }
						...
						*/
						...
						if (isa<CallInst>(II->getOperand(0)))
						{
							...
							TheState.ClassifyPointerVariable(II->getOperand(0), NesCheck::VariableStates::Dyn);
                            // not sure why this is classified
                            /*
                            // THX starts
                            {
                            errs() << "This is the information for load instruction \t" << "\n";
                            ...
                            errs() << " ### source code line : " << location.getLine() << "\n";
                            errs() << " ### source code column : " << location->getColumn() << "\n";
                            std::string record = location->getFilename().str() + "-CALL-DYN-" + II->getFunction()->getName().str()
                                + "-" + std::to_string(location.getLine()) + "-" +  std::to_string(location->getColumn());
                            errs() << " **** record : " << record << "\n";
                            records.insert(record);
                            }
